        ...
        function addComment(urlPath, textAreaId, nb) {
            $.ajax({
                ...
                success: function(result) {
                    ...
                    commentData += '                <span class="author-text">' + result.comment.user.name +
                        '</span>';
                    commentData += '                <span class="published-time-text">' + result.dateDiff +
                        '</span>';
                    commentData += '            </div>';
                    commentData += '            <div class="comment-content" id="comment-content-' + result
                        .comment.id + '">' + nl2br(escapeHtml(result.comment.content));
                    commentData += '            </div>';
                    commentData += '            <div class="d-flex">';
                    commentData += '                <div class="edit-button">';
                    commentData +=
                        '                    <a data-toggle="modal" data-target="#editCommentModal-' + result
                        .comment.id + '">Modifier</a>';
                    commentData += "                </div>";

                    commentData += '                <div class="modal fade" id="editCommentModal-' + result
                        .comment.id + '" tabindex="-1" role="dialog" aria-labelledby="editCommentModal-' +
                        result.comment.id + 'Title" aria-hidden="true">';
                    commentData +=
                    ...
                    commentData +=
                        '                                <textarea class="w-100" name="content" id="edit-comment-' +
                        result.comment.id + '"rows="10">' + result.comment.content + '</textarea>';
                    commentData += '                            </div>';
                    commentData += '                            <div class="modal-footer">';
                    commentData +=
                        '                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Fermer</button>';
                    commentData +=
                        '                                <button type="button" class="btn btn-primary" onclick="saveChanges(\'/comments/' +
                        result.comment.id + "', " + result.comment.id + ')">Enregistrer</button>';
                    commentData += '                            </div>';
                    commentData += '                        </div>';
                    commentData += '                    </div>';
                    commentData += '                </div>';
                    ...
                    commentData += '                    <a onclick="if(confirm(' +
                        "'Voulez vous vraiment supprimer votre commentaire ?')) window.location = '/comments/" +
                        result.comment.id + "/destroy'" + '">SUPPRIMER</a>';
                    commentData += "                </div>";
                    ...
                    if (nb == 1) {
                        ...
                        if ($('#text-' + result.comment.commentable_id).text().includes('Afficher'))
                            $('#text-' + result.comment.commentable_id).text('Afficher les ' + nb +
                            ' réponses');
                        else
                ...
                <div class="comment-body d-flex flex-column w-100">
                    <div class="comment-author d-flex">
                        ...
                        */
                        ...
                        else if ($days > 1 && $days < 7) { $name="il y a $days jours" ; } else { $name="aujourd'hui à " .
                    ...
                    <div class="d-flex">
                        ...
                        @can('delete', $comment)
                            <div class="delete-button">
                                <a
                                    onclick="if(confirm('Voulez vous vraiment supprimer votre commentaire ?')) window.location = '/comments/{{ $comment->id }}/destroy'">SUPPRIMER</a>
                            </div>
                        @endcan
                    </div>
                    ...
                    <form class="mb-3 d-none" id="replyComment-{{ $comment->id }}">
                        ...
                        <div id="add-reply-{{ $comment->id }}" class="add-comment-btn-box justify-content-end">
                            ...
                            <input id="submit-reply-{{ $comment->id }}" class="add-comment-btn add-comment-btn-submit"
                                type="button" disabled value="RÉPONDRE"
                                onclick="addComment('/commentsReply/{{ $comment->id }}', '#text-comment-reply-{{ $comment->id }}', {{ $comment->comments->count() }})">
                        </div>
                    </form>
