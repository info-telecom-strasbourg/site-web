        ...
        <div class="container pt-3">
            ...
            <form>
                <div class="input-group md-form form-sm form-2 pl-0">
                    <input class="form-control my-0 py-1 lime-border" type="search" name="search" id="search"
                        placeholder="Rechercher" aria-label="Rechercher" value="{{ $search }}">
                ...
                <div class="row">
                        ...
                        <select class="form-control" name="pole" id="pole">
                            ...
                            @isset($poles)
                                @foreach ($poles as $pole)
                                    <option value="{{ $pole->id }}" @if ($filters[0] == $pole->id)
                                        selected
                        ...
                        <select class="form-control" name="membre" id="membre">
                            ...
                            @isset($participants)
                                @foreach ($participants as $participant)
                                    <option value="{{ $participant->id }}" @if ($filters[1] == $participant->id) selected
                                @endif>{{ $participant->name }}</option>
                                @endforeach
                        ...
                        <select class="form-control" name="partner" id="partner">
                            ...
                            @isset($partners)
                                @foreach ($partners as $partner)
                                    <option value="{{ $partner->id }}" @if ($filters[2] == $partner->id) selected
                    ...
                    <div class="col-md-3">
                        <select class="form-control" name="trie" id="trie">
                            ...
                            <option value="1" @if ($filters[3] == 1) selected </beautify
                                    end="@endif">>Ordre alphabétique</option>
                            <option value="2" @if ($filters[3] == 2) selected </beautify
                                    end="@endif">>Ordre alphabétique inverse</option>
                            <option value="3" @if ($filters[3] == 3) selected </beautify
                                    end="@endif">>Date de création</option>
                            <option value="4" @if ($filters[3] == 3) selected </beautify
                                    end="@endif">>Date de création inverse</option>
            ...
            <div class="container pt-5">
                @include('partials.list-cards', ['items' => $projets, 'errorMessage' => "Aucun projet n'a été trouvée",
                ...
                @can('create', 'App\Projet')
                    <div class="text-center" style="margin-top:25px; margin-bottom:25px;">
                        <button type="submit" class="btn btn-primary btn-rounded"
                            onclick="self.location.href='/projets/create'">Créer un projet</button>
                    </div>
                @endcan
            </div>
